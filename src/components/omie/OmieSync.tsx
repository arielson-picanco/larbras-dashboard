...
export function OmieSync() {
  ...
  async function handleRunSync() {
    ...
    try {
      ...
      showToast('Sincronização iniciada! Acompanhe o histórico abaixo.')
  ...
  const btnPrimary: CSSProperties = {
    ...
    color: '#0b0d18', cursor: 'pointer', transition: 'opacity .15s',
    fontFamily: 'inherit',
  }
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '1.5rem 0' }}>
      ...
      {toast && (
        <div style={{
          ...
          background: toast.ok ? 'var(--success, #4ade80)' : 'var(--danger, #f06b6b)',
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 6 }}>
          Sincronize pedidos faturados do Omiê com o banco de dados do dashboard.
          O sync automático roda diariamente às 06:00 (Manaus).
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14, marginBottom: '1.5rem' }}>
        {[
          ...
          },
          {
            ...
            label: 'Status atual',
            value: runningLog ? 'Executando…' : 'Aguardando',
          },
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: '1.25rem' }}>
          <button
            ...
            disabled={running || !!runningLog}
            style={{ ...btnPrimary, opacity: (running || runningLog) ? .6 : 1 }}
          >
            <Play size={14} />
            {running ? 'Iniciando…' : 'Sync Incremental'}
          </button>

          <button onClick={handleTest} disabled={testLoading} style={{ ...btnSecondary, opacity: testLoading ? .6 : 1 }}>
            <Wifi size={14} />
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            ...
            <button
              ...
              disabled={rangeRunning || !!runningLog}
              style={{ ...btnPrimary, opacity: (rangeRunning || runningLog) ? .6 : 1 }}
            >
              <Calendar size={14} />
          ...
          <p style={{ fontSize: 11, color: 'var(--text-tertiary)', margin: '8px 0 0' }}>
            Use para reprocessar pedidos de um período específico. Dados existentes serão atualizados (upsert).
          </p>
        </div>
      </div>
      ...
      {testResult && (
        <div style={{ ...cardStyle, border: `1px solid ${testResult.ok ? 'rgba(74,222,128,.3)' : 'rgba(240,107,107,.3)'}` }}>
          ...
          {testResult.ok && (
            <>
              <div style={{ display: 'flex', gap: 20, fontSize: 13, color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                <span>📦 <strong style={{ color: 'var(--text-primary)' }}>{testResult.total_pedidos}</strong> pedidos (últimos 30 dias)</span>
                <span>📄 <strong style={{ color: 'var(--text-primary)' }}>{testResult.total_paginas}</strong> página(s)</span>
                {testResult.sample_converted && (
                  <span>✅ <strong style={{ color: 'var(--text-primary)' }}>{testResult.sample_converted.length}</strong> itens no 1º pedido</span>
                )}
              </div>
                      ...
                      <thead>
                        <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                          {['data','cliente','vendedor','produto','marca','valor','qtd','bairro','omie_id'].map(k => (
                            <th key={k} style={{ padding: '6px 10px', textAlign: 'left', color: 'var(--text-secondary)', fontWeight: 600, whiteSpace: 'nowrap' }}>
                              {k}
                            </th>
                          ))}
                        </tr>
                          ...
                          <tr key={i} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                            {['data','cliente','vendedor','produto','marca','valor','quantidade','bairro','omie_id'].map(k => (
                              <td key={k} style={{ padding: '6px 10px', color: 'var(--text-primary)', whiteSpace: 'nowrap', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {k === 'valor'
                                  ? `R$ ${Number(row[k]).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
                                  : String(row[k] ?? '—')}
                              </td>
                            ))}
                          </tr>
      ...
      <div style={cardStyle}>
        ...
        {loading ? (
          ...
          <div style={{
            ...
            <br />Nenhum sync executado ainda.<br />Clique em "Sync Incremental" para iniciar.
          </div>
        ) : (
              ...
              <div key={log.id} style={{
                ...
                {expandLog === log.id && (
                  <div style={{
                    ...
                    {[
                      ['Período buscado',  `${fmtDateOnly(log.date_from)} → ${fmtDateOnly(log.date_to)}`],
